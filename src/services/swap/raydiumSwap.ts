...
import { config, request, utils } from '../../core/init'; // Import the configuration
...
class RaydiumSwap {
  ...
  async analyse(value: VersionedTransactionResponse) {
    ...
    var preToken: TokenBalance,
      postToken: TokenBalance,
      preSol: TokenBalance,
      postSol: TokenBalance;
  ...
  async telegramNotice(
    ...
    msg =
      msg + '上链时间：' + now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds() + ' \n';
    ...
    msg =
      msg + '💰 钱包余额：' + chat_id + ' ' + solNumber + ' SOL ($' + myValue.toFixed(2) + ')\n\n';
    if (ifsave) {
      await redis.rpush(
        chat_id + ':trade:' + txinfo.token,
        JSON.stringify({ amount: txinfo.amount, price, signature, type: txinfo.type, number, fee })
      );
  ...
  async swapRaydium(
    ...
    const quoteAmountSeed = new TokenAmount(
      new Token(TOKEN_PROGRAM_ID, new PublicKey(config.wsol), decimal),
      SeedAmount,
      false
    );
    transaction.add(
    ...
    transaction.add(
      createInitializeAccountInstruction(associatedWSol, new PublicKey(config.wsol), owner)
    );
    ...
    if (type == 'buy') {
      ...
      toToken =
        poolKeys!.baseMint.toString() == config.tokenAAddress
    ...
    const quoteAmount = new TokenAmount(
      new Token(TOKEN_PROGRAM_ID, new PublicKey(fromToken), decimal),
      amount,
      false
    );
    console.log(
      ...
      decimal,
      amount,
      associatedAddressIn.toString(),
      associatedAddressOut.toString()
    );
    ...
    // 关闭账号
    transaction.add(createCloseAccountInstruction(associatedWSol, owner, owner));
  ...
  async hashDetail(
    value: VersionedTransactionResponse,
    signer: string,
    token: string,
    solprice: number
  ) {
  ...
  //增强Grpc的参数解析
  async analyseatlas(value: {
    transaction: { message: { accountKeys: any; instructions: any }; signatures: string[] };
    meta: {
      err: null;
    ...
    compiledInstructions.forEach(
      (element: { programId: string; accounts: string | any[] }, i: any) => {
    ...
    postTokens.forEach(
      (val: { owner: string; mint: string; uiTokenAmount: { amount: any; decimals: number } }) => {
        if (val.owner == txType.ammAuthority) {
          if (val.mint == useMint) {
            if (useMint == config.wsol) {
    ...
    try {
      if (beforeWsol != undefined && pollWsol != undefined) {
        txType.sol = new Decimal(beforeWsol)
          .sub(new Decimal(pollWsol))
          .div(new Decimal(1000000000))
          .toFixed(9);
        txType.sol_reserves = pollWsol;
      }
      if (beforeUsdt != undefined && pollUsdt != undefined) {
        txType.usdt = new Decimal(beforeUsdt)
          .sub(new Decimal(pollUsdt))
