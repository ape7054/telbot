import { followMenu, flowMenu, tokenMenu, snipeDetailMenu, settingMenu, snipeMenu, snipeAutoMenu, noUserMenu, tokensMenu, analyseTokenMenu } from './menu';
...
export default class BotFun {
    ...
    */
    async menuSell(token:string, userid:string, amount:number){
      ...
      if(ammid){
        ...
        if(redisInfo){
          ...
          var result:any = await tapchain.pumpSwapSell(userid, token, amount, 0, '', bonding_curve, associated_bonding_curve);
          if(result.error) return {msg,error:'卖失败:'+result.error};
          hash = result.hash;
          msg = result.error;
          time = result.time;
      ...
      await redis.rpush("botbuysell",JSON.stringify({address,time,bundle,hash,type:'sell',amount}));
      if(hash) {
        msg = "✅卖出请求发送成功！<a href='https://solscan.io/tx/"+hash+"'>点击查看Hash</a>\n\n";
        await redis.publish("hashcheck",hash);
        await redis.set("teleNotice:"+hash,JSON.stringify({chat_id:userid,type:'buy',address,time}),'EX',600);
      }
      if(bundle) msg = "✅卖出请求发送成功！<a href='https://explorer.jito.wtf/bundle/"+bundle+"'>点击查看Jito</a>\n\n";
      return {msg,error:''};
    }
    ...
    async menuBuy(token:string,userid:string,amount:number){
      ...
      if(ammid){
        ...
        if(pumpRes){
          ...
          var result:any = await tapchain.pumpMenuBuy(userid,token,amount,bonding_curve, associated_bonding_curve);
          console.log(result);
          if(result.error){
      ...
      await redis.rpush("botbuysell",JSON.stringify({address,time,bundle,hash,type:'buy',amount}));
      if(hash) {
        msg = "✅购买请求发送成功！<a href='https://solscan.io/tx/"+hash+"'>点击查看Hash</a>\n\n";
        await redis.publish("hashcheck",hash);
        await redis.set("teleNotice:"+hash,JSON.stringify({chat_id:userid,type:'buy',address,time}),'EX',600);
      }
      if(bundle) msg = "✅购买请求发送成功！<a href='https://explorer.jito.wtf/bundle/"+bundle+"'>点击查看Jito</a>\n\n";
      return {msg,error:''};
    }
    ...
    async mySetting(ctx:any,fromId:number){
      ...
      var setting  = {fetchOpen:true,jitoOpen:false,swapGas:0.001,swapSlippage:100,holdBuy:1,jitoFee:0.0025};
      if(obj){
      ...
      try {
        ...
        "🟢交易优先费："+setting.swapGas+"SOL\n"+
        "🟢交易滑点："+setting.swapSlippage+"%\n"+
        "🟢贿赂消费："+setting.jitoFee+"%\n";
    ...
    async snipeAuto(ctx:any,fromId:number){
      ...
      try {
        ...
        msg=msg+"🟢Gas费"+setting.gas+"SOL\n"+
        "🟢单次狙击金额："+setting.maxSol+"SOL\n"+
        "位置模式\n"+
        "🔫最高买入位置："+setting.maxBuyPosition+"位\n"+
        "🔫交易笔数最大值："+setting.maxSwapPosition+"笔\n"+
        "🔫最少夹几笔买单："+setting.fastSell+"笔\n"+
        "时间模式\n"+
        "🔫买入后"+setting.longtime+"秒涨幅目标"+setting.longbili+"%\n"+
        "🔫达成目标后卖出"+setting.longsell+"%\n"+
        "🔫买入后持续"+setting.longforce+"秒强制清仓";
      } catch (error) {
        var saveSet  = {
          longtime:5,longbili:100,longsell:100,longforce:10,
          maxSol:0.1,fastSell:0,maxBuyPosition:2,maxSwapPosition:8,status:0,gas:0.001
        };
        await redis.set(fromId+":snipeConfig",JSON.stringify(saveSet));
        msg=msg+"🟢Gas费"+saveSet.gas+"SOL\n"+
        "🟢单次狙击金额："+saveSet.maxSol+"SOL\n"+
        "位置模式\n"+
        "🔫最高买入位置："+saveSet.maxBuyPosition+"位\n"+
        "🔫交易笔数最大值："+saveSet.maxSwapPosition+"笔\n"+
        "🔫最少夹几笔买单："+saveSet.fastSell+"笔\n"+
        "时间模式\n"+
        "🔫买入后"+saveSet.longtime+"秒涨幅目标"+saveSet.longbili+"%\n"+
        "🔫达成目标后卖出"+saveSet.longsell+"%\n"+
        "🔫买入后持续"+saveSet.longforce+"秒强制清仓";
      }
      ...
      for (let i = 1; i < snipes.length+1; i++) {
        ...
        if(obj){
          try{
            ...
            msg=msg+i+". "+token.slice(0,5)+"         "+info.amount+"SOL            --\n";
          } catch (error) {
    ...
    async getMytokens(address:string){
      ...
      var result = await solana.getTokenAccountsByOwner(
  ...
  async tokeninfoByMoralis(token:string){
    const client = axios.create({
      headers: {
        ...
        'X-API-Key': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJub25jZSI6IjNmZWY5ZTVhLTY1OWYtNDJiMy1iYjZmLTNkNmZhNTg2NmEwYiIsIm9yZ0lkIjoiNDA3Mzc1IiwidXNlcklkIjoiNDE4NjAzIiwidHlwZUlkIjoiYjU4OTUzZDQtNjYyMC00YjNlLTg2MDAtOTQ4MjJhYjZjNjA3IiwidHlwZSI6IlBST0pFQ1QiLCJpYXQiOjE3MjU2MzYyMTcsImV4cCI6NDg4MTM5NjIxN30.WfZunxl2PYN4oLB8GYQEpbcIKc_hXHegHnEt1AAmxDM'
      },
    });
    try {
      const respssonse = await client.get('https://solana-gateway.moralis.io/token/mainnet/'+token+'/metadata');
      if(respssonse.status==200){
        ...
        return {name:metadata.name,decimals,symbol:metadata.symbol,updateAuthority:metadata.updateAuthority}
      }
    }catch{
  ...
  async detail_token(ctx:any,address: string | null,fromId:number,token: string | null){
    ...
    if(MyTokenAddress){
      ...
      if(account){
        ...
        tokenBalance = new Decimal(accountInfo.amount.toString()).div(10**decimals).toFixed(decimals);
        ...
        if(associated_bonding_curve){
          ...
          if(accountTotal){
            ...
            var totalToken = new Decimal(accountInfo2.amount.toString()).div(10**decimals).toFixed(decimals);
    ...
    if(raydium){
      ...
      const response = await axios.get('https://api-v3.raydium.io/pools/info/mint?mint2=So11111111111111111111111111111111111111112&poolType=standard&poolSortField=liquidity&sortType=desc&pageSize=1&page=1&mint1='+token);
  ...
  async getTokenData(token:string, chat_id:number){
    ...
    msg += '📣 总盈1亏：'+yingli.toFixed(9)+' SOL ($'+(Number(yingli)*config.solprice).toFixed(2)+')\n';
  ...
  //删除跟单
  async delBank(fromId: number | undefined, editadd: string, ctx: { reply: (arg0: string) => void; }){
    var address = await redis.get(fromId+":address") || '';
  ...
  async snipeDetail(ctx:any,token:string,fromId:number,address:string){
    ...
    var msg = "📌"+token.slice(0,5)+"狙击设置成功\n"+
    ""+token+"\n"+
    "💡 状态："+status+"\n"+
    "🔫 狙击金额："+info.amount+" SOL\n"+
    "💵 狙击优先费："+info.snipeGas+" SOL\n"+
    "💰 钱包余额："+balance+" SOL"+add+"\n"+
