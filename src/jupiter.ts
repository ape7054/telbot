...
export class JupiterSwap {
    ...
    async swapone(tx:string,swaptoken:string,amount:number,route:RouterInfo,beforedex:string,afterdex:string){
        ...
        if(startInputMint != wsolstr && startInputMint != usdtstr){
            if(routes.length==1){
                if(routes[0].output==wsolstr || routes[0].output == usdtstr){
                    ...
                    routes = [
                        {
                            name:"Lifinity V2",amm:"",id:0,poola:"",poolb:"",in:"",out:"",
                            input:wsolstr,
                            inpool:"",
                            outpool:"",
                        ...
                        {
                            name:"Raydium",amm:"",id:0,poola:"",poolb:"",in:"",out:"",
                        ...
                        {
                            name:"SolFi",amm:"",id:0,poola:"",poolb:"",in:"",out:"",
    ...
    async realswap(amount:number,routes:RouterInfo[]){
        ...
        for (let i = 0; i < routes.length; i++) {
            ...
            const [response] = await Promise.all([
                axios.get(process.env.JUPITER_API_URL+"/quote", {
                    params: {
        ...
        if(feeAmount.gt(new Decimal(10000))){
            ...
            const [ swapInstructions ] = await Promise.all([
                axios.post(process.env.JUPITER_API_URL+"/swap-instructions", {//请求合并后的swap指令
            ...
            const tx = new TransactionInstruction({
                keys: accounts.map((account: { pubkey: string; isSigner: boolean; isWritable: boolean; }) => ({
