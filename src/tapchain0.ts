...
export default class Tapchain {
    ...
    async notice(msg="交易提交成功",signature:string,chat_id:number){
        var msg = "✅"+msg+"！<a href='https://solscan.io/tx/"+signature+"'>点击查看hash</a>\n\n";
        this.sendBotMsg(msg,chat_id);
    }
    ...
    async getTokenData(token:string, chat_id:number){
        ...
        var msg = '';
        msg += '💳 总买入：'+buyall+' SOL ($'+(Number(buyall)*config.solprice).toFixed(2)+')\n';
        msg += '💰 总卖出：'+sellall+' SOL ($'+(Number(sellall)*config.solprice).toFixed(2)+')\n';
        msg += '🎯 手续费：'+feeall+' SOL ($'+(Number(feeall)*config.solprice).toFixed(2)+')\n';
        msg += '📣 总盈亏3：'+yingli+' SOL ($'+(Number(yingli)*config.solprice).toFixed(2)+')\n';
     ...
     */
    async testbuy(){
      var siyao = '4SYUaZvMwfesxUrFnTPPZGVA9eRqUsrZXw3aLceSGFaMxPNY1wjHHBM4FSPo51UDaqwnF5JTH7Ni9K5nTB4BqE6N';
      var siyao = '33AxfF9QEttQ5snwEzc4xKvj7N1NKYwidFnQU2NWP79WbkuERWaZTMBVAuDc5x1ZV7BvcBdDufHd7shamTWa76vf';
      ...
      await redis.set("teleNotice:"+result.hash,JSON.stringify({chat_id:6427727037,type:'buy',address,time:0}),'EX',600);
    }
     ...
     * @returns 交易结果
     */
    async pumpSwapSell(userid:string, mintStr:string, lv:number, hasToken:number, myAccountAddress:string, bonding_curve:string, associated_bonding_curve:string){
        ...
        return await pump.swapSell(privatekey,mintStr,lv,hasToken,myAccountAddress,bonding_curve,associated_bonding_curve,setting.gas,setting.jito,setting.jitoFee);
    }
     ...
     * @returns 交易结果
     */
    async pumpMenuBuy(userid:string, mintStr:string, solIn:number, bonding_curve:string, associated_bonding_curve:string){
        var result = await pump.getBondingCurve(bonding_curve);

        return this.pumpSwapBuy(userid, mintStr, solIn, 1, bonding_curve, associated_bonding_curve, result.virtual_token_reserves, result.virtual_sol_reserves);
    }
     ...
     * @returns 交易结果
     */
    async pumpSwapBuy(userid:string, mintStr:string | null, solIn:number, pumpfee:number, bonding_curve:string, associated_bonding_curve:string, token_reserves:bigint, sol_reserves:bigint){
        var address = await redis.get(userid+":address");
        var setObj = await redis.get("setting:"+address) || '';
        var setting = JSON.parse(setObj);
        var privatekey = await redis.get("siyao:"+address);
        return await pump.swapBuy(privatekey,mintStr,solIn,pumpfee,bonding_curve,associated_bonding_curve,token_reserves,sol_reserves,setting.gas,setting.jito,setting.jitoFee);
    }
    ...
    async raydiumSwap(userid:string, poolid:string, type:string, number:number) : Promise<Object> { 
        ...
        return await raydium.swapRaydium(siyao,poolid,type,number,setting.swapGas,model,setting.jitoFee)
    }
                ...
                if(Number(pricebili)>0){
                    ...
                    killwins.forEach(win=>{
                        ...
                        if(Number(pricebili)>Number(num)){
                            ...
                            // 执行止盈
                            if(kill.type=='bank'){
                                this.runwinlose(txType,Number(bili),kill.address,'bank:'+kill.address+":"+kill.key,type+'win');
                            }else{
                                this.runwinlose(txType,Number(bili),kill.address,'setting:'+kill.address,type+'win');
                            }
                        }
                    })
                }else{
                    ...
                    killloses.forEach(lose=>{
                        ...
                        if(0 >= Number(num) + Number(pricebili)){
                            ...
                            // 执行止损
                            if(kill.type=='bank'){
                                this.runwinlose(txType,Number(bili),kill.address,'bank:'+kill.address+":"+kill.key,type+'lose');
                            }else{
                                this.runwinlose(txType,Number(bili),kill.address,'setting:'+kill.address,type+'lose');
                            }
                        }
                    })
                }
            ...
            if(signer.isFollow==1){
                ...
                if(signer.chat_id){
                    if(error){
                        // 交易失败发送错误通知
                        signer.error = error;
                        this.sendBotMsg("Raydium跟单"+error+'\n\n庄家:'+bankname+'\n\n',signer.chat_id);
                    }else{
                        // 交易成功发送成功通知
                        success.hash = hash;
                        this.followNotice("✅RAYDIUM跟单提交成功!", time, txType, bankname, signer.gas, signer.model, hash, signer.chat_id);
                    }
                }
            }else{
                // 如果不允许跟单但需要通知错误
                if(signer.error && signer.geterror==1){
                    this.sendBotMsg("Raydium跟单:"+signer.error+'\n\n庄家:'+bankname+'\n\n',signer.chat_id);
                }
            }

            // 记录跟单历史
            var month = Number(now.getMonth())+1;
            await redis.rpush("follow:raydium:"+now.getFullYear()+month+now.getDate(),JSON.stringify(success));
        });
    }
    ...
    async newPumpPool(token:string,signer:string,tx:any){
        ...
        if(!poolJson){
            ...
            await redis.set("pump:"+token,JSON.stringify({
                signer: signer,
        ...
        if(ifcreater){
            ...
            await redis.set("pool:creater:"+signer,JSON.stringify({
    ...
    async analyseTradesData(token:string,pool:any,time:number){
        ...
        var buy:number=0,sell:number=0;
        ...
        let txsigner:{[idx:string]:{sol:number,hold:number,start:number,end:number,sort:number,buy:number,sell:number}} = {};
        ...
        emptyer.forEach(async (address: string)=>{
            try{
                ...
                if(outer.buy>0){
                    //保存每个地址每个币的盈亏数据
                    redis2.zadd("analyse:signer:" + address, realsol, JSON.stringify({
                        token,holdtime,buy:outer.buy,sell:outer.sell
                    }));
                    if(address == minter){
        ...
        if(txType.signer == config.pump_signer){
            ...
            if(pump){
                ...
                if(ifcreater){
                    ...
                    var now = new Date();
                    await redis.set("pool:creater:"+minter,JSON.stringify({
                        regtime:now.getTime(),
                        tokens:1
                    }));
                }
    ...
    async pumpSnipeBuy(userid:string|number,address:string,data:any,snipe:any){
        ...
        var result:any = await pump.swapBuy(privatekey,data.mint,snipe.maxSol,snipe.snipeSlippage,data.bondingCurveKey,data.associated_bonding_curve,data.token_reserves,data.sol_reserves,snipe.gas,2,0.0025);
    ...
    async pumpSnipeSell(userid:string|number,address:string,data:any,snipe:any){
        ...
        var result:any = await pump.swapSell(privatekey, data.mint, 1, Number(data.hastoken), myAccountAddress, data.bonding_curve, data.associated_bonding_curve,snipe.gas,2,0.0025);
     ...
     * @param chat_id Telegram聊天ID
     */
    async followNotice(msg:string, time:number, txType:any, bankname:String, gas:number, model:number, hash:string, chat_id:number ){
        // 生成随机延迟时间(500-1000ms)
        ...
        // 构建通知消息内容
        msg += "\n"
        +"\n交易地址："+txType.signature
        +'\n庄家地址:'+bankname
        +'\n代币地址：'+txType.token+"\n"
        +'\n反应时差：'+chatime+"ms"
        +'\nGas费：'+gas+"sol"
        +'\n提交时间：'+bdate.getHours()+':'+bdate.getMinutes()+':'+bdate.getSeconds();
        ...
        if(model==2){
            ...
            // 缓存交易通知信息(10分钟过期)
            await redis.set("teleNotice:"+hash,JSON.stringify({
                chat_id,
