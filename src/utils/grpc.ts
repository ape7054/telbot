...
async function loadData() {
  ...
  stream.on('data', (data: { transaction: { transaction: any; slot: any }; filters: string[] }) => {
    ...
    if (data?.transaction && data.filters[0] == 'pumpFun') {
      ...
      var bondingCurveIndex: any,
        associatedBondingCurveIndex: any,
        tokenIndex = 0;
      ...
      instructions.forEach(
        (element: {
          ...
          accounts: { toJSON: () => { (): any; new (): any; data: any } };
        }) => {
          // 查找Pump程序的指令
      ...
      if (value.meta.preTokenBalances.length == 0) {
        ...
        value.meta.preTokenBalances.forEach(
          (val: { accountIndex: any; uiTokenAmount: { amount: number; decimals: number } }) => {
      ...
      if (value.meta.postTokenBalances.length == 0) {
        ...
        value.meta.postTokenBalances.forEach(
          (val: {
            accountIndex: number;
            owner: string;
            mint: string;
            uiTokenAmount: { amount: number; uiAmount: number; decimals: number };
          }) => {
            if (val.accountIndex == associatedBondingCurveIndex) {
      ...
      if (decimals > 0) {
        // 计算SOL变化量(单位：SOL)
        hashInfo.sol = new Decimal(beforeSol)
        ...
        if (iscreate == 1) {
          // 如果是创建池子的交易
          // 计算代币数量变化
          hashInfo.number = new Decimal(pollToken)
          ...
          // 如果是交易
          // 计算代币数量变化
          hashInfo.number = new Decimal(beforeToken)
    ...
    if (data?.transaction && data.filters[0] == 'raydiumFun') {
      ...
      // 遍历账户列表查找各种程序的索引位置
      var rayProgramIndex = -1,
        tokenProgramIndex = -1,
        lifinityProgramIndex = -1;
      accounts.forEach((val, i) => {
        // 第一个账户为交易签名者
        if (i == 0) {
          rayInfo.signer = val;
      ...
      if (ammids.length == 1) {
        ...
        postTokens.forEach(
          (val: {
            owner: any;
            mint: string;
            uiTokenAmount: { amount: number; uiAmount: any; decimals: number };
          }) => {
            if (val.owner == config.raydium_authority) {
              if (val.mint == useMint) {
        ...
        // 如果代币有小数位数，计算交易相关数据
        if (decimals > 0) {
          rayInfo.number = new Decimal(beforeToken)
            .sub(new Decimal(pollToken))
            .div(10 ** decimals)
            .toFixed(decimals);
          ...
          if (rayInfo.token_reserves && rayInfo.sol_reserves) {
            rayInfo.price = new Decimal(rayInfo.sol_reserves)
              .div(10 ** (9 - decimals))
              .div(new Decimal(rayInfo.token_reserves))
              .toFixed(15);
            console.log(`交易连接：https://solscan.io/tx/${signature}`);
