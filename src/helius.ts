...
const ws: WebSocket = new WebSocket('https://mainnet.helius-rpc.com/?api-key=d2b9c2e8-adca-4fa5-a152-bd3962cad5a8');
...
const emptyRouterInfo = (): RouterInfo => ({
    ...
    in: "",
    out: "",
    inpool: "",  // 添加缺失的 inpool 属性
    outpool: ""  // 添加缺失的 outpool 属性
    ...
    try {
        ...
        if(instructions.length>0){
            instructions.forEach((item:any,mainindex:number) => {
                ...
                //     })
                // }
                var ins = innerInstructions.find((insx:any) => insx.index === mainindex)?.instructions || [];
                    ...
                    ins.forEach((insitem:any) => {
                        if(amminfo.id>0 && insitem.program == 'spl-token'){
                            if(amminfo.id==5){
                                ...
                                if( insitem.parsed.info.destination != maininsitem.accounts[8] &&
                ...
                if(item.programId == JupiterProgram && item.data && item.accounts[0] == 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'
                    ...
                    ins.forEach((insitem:any) => {
                        ...
                        if(routertype==5){ //stabble Vault Program: withdraw
                            let rizhi = 0;
