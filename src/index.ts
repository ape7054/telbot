...
const solanabig = new Connection(config.RPC_URL, {
  commitment: 'processed',
  disableRetryOnRateLimit: true,
});
...
function createLogListener(accountAddress: string) {
  ...
  // 创建监听器
  const subscriptionId = solana.onLogs(
      ...
      if (log.err == null) {
        ...
        if (!existTx) {
          ...
          if (swaptype == 1 || swaptype == 2) {
            var value = await solana.getTransaction(log.signature, {
...
async function checkTransaction(nowParsed: Set<string>) {
  ...
  if (noParseds.length == 1) {
    var one = noParseds[0];
    solana
      .getTransaction(one, { maxSupportedTransactionVersion: 0, commitment: 'confirmed' })
      .then(async value => {
        if (value == null) {
  ...
  solanabig
    .getTransactions(noParseds, { maxSupportedTransactionVersion: 0, commitment: 'confirmed' })
    .then(async values => {
      try {
