import { Keypair,SystemProgram, PublicKey, Transaction, TransactionInstruction, LAMPORTS_PER_SOL, ComputeBudgetProgram, VersionedTransactionResponse} from '@solana/web3.js';
...
class PumpSwap {
    ...
    async swapBuy(
        ...
        sol_reserves:bigint,
        gas:number,model:number,fee:number
    ):Promise<{error:string,hash?:string,time?:number}>{
        if(privatekey == null || mintStr == null || intosol<=0 || solIn == undefined || sol_reserves == undefined || token_reserves==0n || token_reserves == undefined) return {error:'参数错误'};
        ...
        if(!tokenAccountInfo){
            ...
            await redis.set("associated:"+mintStr+":"+owner.toString(),tokenAccountAddress.toString());
        }
    ...
    async analyse(value:VersionedTransactionResponse){
        const txType = {time:0,token:'',signer:'',signature,bonding_curve:'',associated_bonding_curve:'',type:'error',fee:0,sol:0,number:0,token_reserves:0,sol_reserves:0};
        ...
        var pumpProgramIndex=-1, pumpRentIndex=-1, tokenProgramIndex=-1;
        ...
        var bondingCurveIndex = 0,associatedBondingCurveIndex = 0;
        ...
        if(bondingCurveIndex==0 || associatedBondingCurveIndex==0 || bondingCurveIndex == undefined || associatedBondingCurveIndex == undefined){
        ...
        var pollSol,beforeSol,beforeToken,pollToken,decimals = 0;
        ...
        if(txType.type=='buy') {
            txType.sol = ((new Decimal(pollSol)).sub(new Decimal(beforeSol))).div(new Decimal(1000000000)).toFixed(9);
        ...
        if(txType.type=='sell') {
            txType.sol = ((new Decimal(beforeSol)).sub(new Decimal(pollSol))).div(new Decimal(1000000000)).toFixed(9);
            txType.number = ((new Decimal(pollToken)).sub(new Decimal(beforeToken)).div(10**decimals)).toFixed(decimals);
        }
    ...
    async telegramNotice(signature:string,txinfo:any,chat_id:number,ifsave:boolean){
        ...
        if(price){
          msg +='🎯 价格：'+price+' SOL ($'+(Number(price)*config.solprice).toFixed(12)+')\n';
        }
        msg +='📈 市值：'+sol+' SOL ($'+(Number(sol)*config.solprice).toFixed(2)+')\n';
        msg +='💰 钱包余额：'+solNumber+' SOL ($'+myValue.toFixed(2)+')\n\n';
        var type = txinfo.type;
        if(ifsave){
            await redis.rpush(chat_id+":trade:"+txinfo.token,JSON.stringify({amount,price,signature,type,number,fee}));
     ...
     * @param gas 
     */
    async telegramAuto(signature:string,type:string,token:string,myaddress:string,gas:number){
        var now = new Date();
        var msg = "✅";
        if(type=='buy'){
        ...
        +'代币地址：'+token+"\n"
        +'Gas费：'+gas+"sol\n"
        +"上链时间："+now.getHours()+':'+now.getMinutes()+':'+now.getSeconds();
        const bot = new Bot(config.botapi);
