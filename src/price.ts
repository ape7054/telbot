...
export class PriceOptimizer {
  ...
  processItem(routerInfo: RouterInfo): boolean {
    ...
    if (routerInfo.input == wsolstr) {
      ...
      if (routerInfo.output == usdcstr) {
        // [wsolstr,usdcstr] => [usdcstr,wsolstr]
        newItem.unit = 1;
      ...
      newItem.price = new Decimal(routerInfo.in).div(new Decimal(routerInfo.out)).toNumber();
    } else if (routerInfo.output == wsolstr) {
      // 这里已经排除了 input = USDC
      newItem.unit = 0;
      newItem.mint = routerInfo.input;
      newItem.price = new Decimal(routerInfo.out).div(new Decimal(routerInfo.in)).toNumber();
    } else if (routerInfo.output == usdcstr) {
      // 这里已经排除了 input = WSOL
      newItem.unit = 1;
      newItem.mint = routerInfo.input;
      newItem.price = new Decimal(routerInfo.out).div(new Decimal(routerInfo.in)).toNumber();
    ...
    if (
      newItem.unit == 0 &&
      routerInfo.input != 'So11111111111111111111111111111111111111112' &&
      routerInfo.output != 'So11111111111111111111111111111111111111112'
    )
      console.log(routerInfo, newItem);
    ...
    if (!current) {
      ...
      if (updated) {
        ...
        fs.writeFileSync(
          path.join(this.tokensDir, `tokens/${mint}.json`),
